...
public:
	...
	path(float a, float b, float c, int d) {
		min=a;
		dist_0=b;
		dist_1=c;
		dest=d;
...
void getinput() {
	...
	for (int i = 0; i < n_road; i++) {
		...
		edge[i_start].push_back(road(i_dist, i_end, i_p/100));
		edge[i_end].push_back(road(i_dist, i_start, i_p / 100));
	}
}
...
float dijkstra() {
	...
	path pa(0, 0, basicprice, start);
	pq.push(pa);
    int curr;
	float currdist_0, currdist_1, nextdist_0, nextdist_1;
	while (!pq.empty()) {
		curr = pq.top().dest;
		...
		for (road next : edge[curr]) {
			...
			nextdist_1 = min(currdist_0 + basicprice, currdist_1) + next.dist*price_1;
			...
			if ( min(nextdist_0, nextdist_1) < min(dist[next.dest][0], dist[next.dest][1]) ) {
					...
					pq.push(path(min(dist[next.dest][0], dist[next.dest][1]), dist[next.dest][0], dist[next.dest][1], next.dest));
			}
		}
	}
